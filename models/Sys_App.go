...
func init() {
	orm.RegisterModel(new(SysApp))
}

// AddSysApp insert a new SysApp into database and returns
...
// GetSysAppById retrieves SysApp by Id. Returns error if
// Id doesn't exist
...
// GetAllSysApp retrieves all SysApp matches certain condition. Returns empty list if
// no records exist
func GetAllSysApp(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(SysApp))
	// query k=v
	for k, v := range query {
...
// UpdateSysApp updates SysApp by Id and returns error if
...
// DeleteSysApp deletes SysApp by Id and returns error if
...
func DeleteSysApp(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&SysApp{Id: id}); err == nil {
