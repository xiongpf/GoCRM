...
func init() {
	orm.RegisterModel(new(SysButton))
}

// AddSysButton insert a new SysButton into database and returns
...
// GetSysButtonById retrieves SysButton by Id. Returns error if
// Id doesn't exist
...
// GetAllSysButton retrieves all SysButton matches certain condition. Returns empty list if
// no records exist
func GetAllSysButton(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(SysButton))
	// query k=v
	for k, v := range query {
...
// UpdateSysButton updates SysButton by Id and returns error if
...
// DeleteSysButton deletes SysButton by Id and returns error if
...
func DeleteSysButton(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&SysButton{Id: id}); err == nil {
