...
// AddSysInfo insert a new SysInfo into database and returns
...
// GetSysInfoById retrieves SysInfo by Id. Returns error if
// Id doesn't exist
...
// GetAllSysInfo retrieves all SysInfo matches certain condition. Returns empty list if
// no records exist
func GetAllSysInfo(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(SysInfo))
	// query k=v
	for k, v := range query {
...
// UpdateSysInfo updates SysInfo by Id and returns error if
...
// DeleteSysInfo deletes SysInfo by Id and returns error if
...
func DeleteSysInfo(id int) (err error) {
	...
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&SysInfo{Id: id}); err == nil {
